...
def add_appointment(appointment_input_dict):
    try:
        doctor : Doctor = get_doctor_by_id(appointment_input_dict['id_doctor'])
        if doctor is None:
            raise DoctorNotFoundException()
        patient : Patient = get_patient_by_id(appointment_input_dict['id_patient'])
        if patient is None:
            raise PatientNotFoundException()
        medical_center : MedicalCenter = get_patient_by_id(appointment_input_dict['id_medical_center'])
        ...
        datetime.datetime.fromisoformat(appointment_input_dict['appointment_date']))
        if doctor_medical_appointments is not None:
        ...
        medical_appointment : MedicalAppointment = MedicalAppointment(appointment_date=datetime.datetime.fromisoformat(appointment_input_dict['appointment_date']),
